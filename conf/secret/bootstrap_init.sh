...
then
  ...
  sed -i'' 's/trust_period = "168h0m0s"/trust_period = "0h2m0s"/' ~/.secretd/config/config.toml # less unbound time
  sed -i'' 's/"unbonding_time": "1814400s"/"unbonding_time": "180s"/' ~/.secretd/config/genesis.json # less unbound time
  sed -i'' 's/timeout_propose = "3s"/timeout_propose = "500ms"/' ~/.secretd/config/config.toml
  sed -i'' 's/timeout_prevote = "1s"/timeout_prevote = "500ms"/' ~/.secretd/config/config.toml
  sed -i'' 's/timeout_precommit = "1s"/timeout_precommit = "500ms"/' ~/.secretd/config/config.toml
  sed -i'' 's/timeout_commit = "5s"/timeout_commit = "500ms"/' ~/.secretd/config/config.toml
  ...
  # Add genesis account of relayer
  relayer_mnemonic="iron shine seed find south jelly choice cycle alert defense swear speak swallow impose rapid boat aunt kick silly biology frog save purity club"
  echo $relayer_mnemonic | secretd keys add rly-secret --recover
  secretd add-genesis-account "$(secretd keys show -a rly-secret)" 1000000000000000000uscrt
